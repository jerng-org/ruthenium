...
try { 
    console.log(`index : TOP OF TRY`)
    const { default : rus } = await import('./modules/r-u-s.js')
    console.log(`index : loaded r-u-s`)
    const { default : ruthenium } = await import('./modules/framework/ruthenium.js')
    console.log(`index : loaded ruthenium`)
    const { default : middlewares } = await import('./index-middlewares.js')
    console.log(`index : loaded middlewares`)
    ...
    const initCustomRuntimeClient =  async _ => {
      ...
      console.log(`index : inited the handler`)
  
      initCustomRuntimeClient()
      console.log(`index : inited the custom runtime client`)
}
catch (e) { console.error(`
index : outer 'try' block.`, e) }
